...
import LenderRequestsTable from '../../components/lender/lender-requests/LenderRequestsTable';
import { lenderLogout } from '../../store/StoreIndex';
...
const LenderRequest = () => {
    ...
    const LogoutHandler = () => {
        dispatch(lenderLogout(navigate));
    };
    ...
    return (
        <div style={{ display: 'flex', height: '100vh', width: '100%', border: '10px' }}>
            <Sidebar collapsedWidth='70px' defaultCollapsed className='sidebar-container'>
                <Menu className='mt-5'>
                    <NavLink to='/dashboard/lender' className='menu-item-link'><MenuItem><AiFillDashboard className='menu-icon' /> {t('dashboard_text')}</MenuItem></NavLink>
                    <NavLink to='/dashboard/borrower/lending-requests' className='menu-item-link'><MenuItem><IoMdCash className='menu-icon' /> {t('borrowing_text')}</MenuItem></NavLink>
